...
python llama2/run_safedelta.py \
  ...
  --nsamples 512 \
  --seq_len 1024 \
  --upload_name kmseong/llama2-7b-chat-gsm8k-safedelta-scale0.1_origin_512samples
...
def recovery_safety(
    ...
    **kwargs,
    ...
    # Keep the tokenizer vocabulary unchanged.
    # Safe Delta modifies existing model weights; adding a new PAD token here
    # would require resizing embeddings and would make W_orig/W_sft comparison unsafe.
    ...
    final_model = run_safedelta(
        ...
        nsamples=kwargs.pop('nsamples', 128),
...
@torch.no_grad()
def run_safedelta(align_model, ft_model, tokenizer, s, st_layer_idx, nsamples=128, seq_len=512,
    ...
    # Restore the layer that was temporarily wrapped by Catcher.
    # Use st_layer_idx, not 0, so the code also works when Safe Delta starts
    # from an intermediate layer.
    if hasattr(align_layers[st_layer_idx], 'module'):
        align_layers[st_layer_idx] = align_layers[st_layer_idx].module
    torch.cuda.empty_cache()

    if len(inps) == 0:
        raise RuntimeError('No calibration inputs were captured. Check safe_data_path and tokenizer formatting.')

    # outs = torch.zeros_like(inps)
    outs = [None for _ in range(len(inps))]
...
def main(model_name_align: str = 'ckpts/llama2-7b-chat-hf',
    ...
    recovery_safety(model_name_align, model_name_ft, scale, st_layer,
                    safe_data_path=safe_data_path, nsamples=nsamples, seq_len=seq_len,
